...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("MCTH/QCD_TuneCUETP8M1_13TeV_pythia8_RunIISpring15DR74-Asympt50nsRecodebug_MCRUN2_74_V9A/JetTree_Pt-15to30.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("MCTH/QCD_TuneCUETP8M1_13TeV_pythia8_RunIISpring15DR74-Asympt50nsRecodebug_MCRUN2_74_V9A/JetTree_Pt-15to30.root");
      }
...
void JetTree::Loop(TString TagName, float aPtMin, float aPtMax, 
...
// Operating points from https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagRecommendation76X 
  ...
  {
    //From https://twiki.cern.ch/twiki/bin/view/CMS/BTagCTagger#Dependence_of_the_tagging_effici
    TagCut_CvsL[0] = -0.67 ;
    TagCut_CvsL[1] = 0.05 ;
    TagCut_CvsL[2] = 0.45 ;
    TagCut_CvsB[0] = -0.23 ; 
...
//$$
  ...
  else if( TrigType == "2012" || TrigType == "2015" || TrigType == "2015_Cert" || TrigType == "2015_LooserSel") Year = 2012;
  else
  {
...
//**********************************
  ...
  TH1F* hData_All_nPV         = new TH1F("hData_All_nPV","nb. of PV",35,0.5,35.5);
  ...
  TH1F* hData_nPV             = new TH1F("hData_nPV","nb. of PV",35,0.5,35.5);
...
// Filter event if maxpt < given cut || > given cut
    if( nJet > 0 ) if( Jet_pt[0] < minCutJetPtMax || Jet_pt[0] > maxCutJetPtMax ) continue; // Go to next event
...
//$$
    ...
    if ( Run >= 0 ) 
    {
      ww = ww0 = 1;    // No reweighting for Data
    } else {
      ...
      {
        if( nPV > 0 && nPV < 60 ) ww = ww0 * WeightPU[nPV-1];
        else ww = 0;
...
//----------------------------------------------------------------
    ...
    hData_nPV->Fill( npv , ww );		// pthat and PU reweighted
...
// Jet Probability
      if ( TagName == "JP" )
      {
        if ( Jet_ProbaP[ijet] > 0 ) varPos = 10.*Jet_ProbaP[ijet];
        if ( Jet_ProbaP[ijet] > 0 ) varpos = 10.*Jet_ProbaP[ijet];
        if ( Jet_ProbaN[ijet] > 0 ) varneg = 10.*Jet_ProbaN[ijet];
        for( int i = 0; i < 3; ++i )
        {
          if ( Jet_ProbaP[ijet] > TagCut[i] ) TagPos[i] = true;
          if ( Jet_ProbaN[ijet] > TagCut[i] ) TagNeg[i] = true;
        }
...
// Combined CSVv2
        ...
        if ( Jet_CombIVF_P[ijet] > 0. ) varpos = 25.*Jet_CombIVF_P[ijet];
